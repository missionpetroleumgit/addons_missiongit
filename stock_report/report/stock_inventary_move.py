...
class stock_inventary_move(osv.Model):
    _name = "stock.inventary.move"
    _description = 'Reporte Inventario Real Movimientos'
    ...
    def get_lines_report_wage(self, cr, uid, form):
        ...
        if tipo == 'matriz':
            prod_prod = prod_obj.search(cr, uid, [('active', '=', True)], order='default_code,name_template')
            ...
            for prod in prod_prod_data:
                ...
                egresos = 0
                data = {}
                stock_ing_ids = stoc_mov_obj.search(cr, uid, [('product_id', '=', prod.id), ('location_dest_id', 'in', [12,19,20]), ('date', '>=', date_from), ('date', '<=', date_to), ('state', '=', 'done')])
                stock_egre_ids = stoc_mov_obj.search(cr, uid, [('product_id', '=', prod.id), ('location_id', 'in', [12,19,20]), ('date', '>=', date_from), ('date', '<=', date_to), ('state', '=', 'done')])
                if len(stock_ing_ids) > 0 or len(stock_egre_ids) > 0:
                    stock_ing_data = stoc_mov_obj.browse(cr, uid, stock_ing_ids)
                    stock_egr_data = stoc_mov_obj.browse(cr, uid, stock_egre_ids)
                    for moves in stock_ing_data:
    ...
    def action_excel(self, cr, uid, ids, context=None):
        ...
        linea_der = pycel.easyxf('font: colour black, height 140;'
                                 'align: vertical center, horizontal right;'
                                  'borders: left 1, right 1, top 1, bottom 1;')
        if type_pv == 'matriz':
            title = 'PUNTO VENTA MATRIZ'
            title1 = 'INVENTARIO REAL DE'
        elif type_pv == '12oct':
            title = 'PUNTO VENTA 12 DE OCTUBRE'
            title1 = 'INVENTARIO REAL DE'
        elif type_pv == 'guayaquil':
            title = 'PUNTO VENTA GUAYAQUIL'
            title1 = 'INVENTARIO REAL DE'
        elif type_pv == 'todos':
        ...
        ws.write_merge(5, 5, 1, x0, title1 +" "+ time.strftime('%d/%m/%Y', time.strptime(date_from, '%Y-%m-%d')).upper() +" AL "+ time.strftime('%d/%m/%Y', time.strptime(date_to, '%Y-%m-%d')).upper(), style_cabecera)
        ...
        if type_pv == 'matriz':
            ...
            ws.write(xi, 5, 'PRECIO UNITARIO', style_header)
            ws.write(xi, 6, 'VALOR TOTAL', style_header)
            ...
            ws.write(xe, 5, t_pun, linea_der)
            ws.write(xe, 6, t_tot, linea_der)
        ...
        if type_pv == 'matriz':
            ...
            for linea in lineas:
                ...
                ws.write(xi, 5, linea.get('pre_uni', ''), linea_der)
                ws.write(xi, 6, linea.get('valor_total', ''), linea_der)
                xi += 1
                #totales
...
#                 ws.write(xe, 6, linea.get('t_tot', ''), linea_der)
        ...
        res_model = 'stock.inventary.move'
        id = ids and type(ids) == type([]) and ids[0] or ids
        self.write(cr, uid, ids, {'data':out, 'name':name})
...
#             'parent_id': activity.report_directory_id.id,
    ...
    _columns = {
        ...
        'type_pv':fields.selection([('matriz', 'Matriz'), ('12oct', '12 Octubre'), ('guayaquil', 'Guayaquil'), ('todos', 'Todos')], 'Punto Venta', required=True),
    }
    _defaults = {
        'date_from': lambda * a: time.strftime('2015-07-01'),
    }
stock_inventary_move()