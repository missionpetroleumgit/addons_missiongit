...
class hr_payslip_analysis(osv.Model):
    _name = "hr.payslip.analysis"
    _description = "Pay slip Analysis"
    _auto = False
    _rec_name = 'number'
    _order = 'number'
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'hr_payslip_analysis')
        cr.execute("""
            create or replace view hr_payslip_analysis as (
                select 
                    min(pl.id) as id, 
                ...
                where r.appears_on_payslip = true          
                group by  r.order, paylist_name, number, pl.employee_id, empl.identification_id, empl.bank_account_id, employee_name, l.amount, salary_rule, pl.date_from, pl.date_to
                order by r.order
            )
        """)
