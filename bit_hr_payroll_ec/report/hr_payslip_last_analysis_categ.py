...
class hr_payslip_last_analysis_categ(osv.Model):
    _name = "hr.payslip.last.analysis.categ"
    _description = "Pay Slip Last Analysis by Categories"
    _auto = False
    _rec_name = 'number'
    _order = 'number'
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'hr_payslip_last_analysis_categ')
        cr.execute("""
            create or replace view hr_payslip_last_analysis_categ as (
                select 
                    min(pl.id) as id, 
                ...
                inner join  hr_salary_rule_category c on r.category_id = c.id
                where pl.date_from = (SELECT DISTINCT ON (date_from) date_from from hr_payslip order by date_from desc limit 1) 
                and r.appears_on_payslip = true and c.id = 25          
                group by  r.order, paylist_name, number, pl.employee_id,empl.identification_id, empl.bank_account_id, employee_name, l.amount, salary_rule, category, pl.date_from, pl.date_to
                order by r.order
            )
        """)
