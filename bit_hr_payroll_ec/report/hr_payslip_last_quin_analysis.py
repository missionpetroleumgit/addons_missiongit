...
class hr_payslip_last_quin_analysis(osv.Model):
    _name = "hr.payslip.last.quin.analysis"
    _description = "Analisis de Quincena"
    _auto = False
    _rec_name = 'number'
    _order = 'number'
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'hr_payslip_last_quin_analysis')
        cr.execute("""
            create or replace view hr_payslip_last_quin_analysis as (
                select 
                    min(pl.id) as id, 
                ...
                where pl.date_from = (SELECT DISTINCT ON (date_from) date_from from hr_payslip where type  = 'quincena' order by date_from desc limit 1) 
                and r.appears_on_payslip = true
                and pl.type = 'quincena'             
                group by  r.order, paylist_name, number, pl.employee_id,empl.identification_id, empl.bank_account_id, employee_name, l.amount, salary_rule, pl.date_from, pl.date_to, emp_modo_pago
                order by r.order
            )
        """)
