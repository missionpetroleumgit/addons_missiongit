...
# PERIOD
class account_bit_period(osv.osv):
    ...
    def generate_XML(self, cr, uid, ids, doc, filtro, context):
        is_ats = not filtro or False
        form = doc[2:]
        sust = doc[1:]
        period = self.browse(cr, uid, ids, context)
    ...
    # CABECERA FIJA
        ...
        self.load_doc(cr, uid, period.id, data_fname, archivo, res_model)
    
    def action_form103(self, cr, uid, ids, context=None):
        doc = 'F103'
        filtro = [('form', '=', '103'), ('code', '!=', False)]
        self.generate_XML(cr, uid, ids, doc, filtro, context)
    
    def action_form104(self, cr, uid, ids, context=None):
        doc = 'F104'
        filtro = [('form', '=', '104'), ('code', '!=', False)]
        self.generate_XML(cr, uid, ids, doc, filtro, context)
    ...
    def getTagCompra(self, cr, uid, brw_invoices):
        ...
        for invoice in brw_invoices:
            ...
            if invoice.partner_id.part_type not in dicc_type_prov:
                raise osv.except_osv(_('Error!'), 
                                    _('Existe un proveedor que no tiene definido el Tipo de identificación para %s' %(invoice.partner_id.name)))
...
#CSV-24-08-2016/AUMENTO PARA ACTUALIZAR LAS BASES GRABADAS Y NO GRABADAS Y RETENCIONES DE IVA
    ...
    def load_doc(self, cr, uid, period_id, data_fname, archivo, res_model):
...
#             'parent_id': activity.report_directory_id.id,
        ...
        filtro = [('res_id', '=', period_id), ('name', '=', data_fname)]
        att_ids = self.pool.get('ir.attachment').search(cr, uid, filtro)
        if att_ids:
            self.pool.get('ir.attachment').write(cr, uid, att_ids, attach_vals)
        else:
