...
class account_common_report2(osv.osv_memory):
    ...
    _columns = {
        ...
        'level_report': fields.selection([('one', 'Nivel 1'), ('two', 'Nivel 2'), ('three', 'Nivel 3'),
                                          ('four', 'Nivel 4'), ('five', 'Nivel 5'), ('six', 'Nivel 6')], 'Nivel',
                                         required=True)
    }
    ...
    def check_report2(self, cr, uid, ids, context=None):
        ...
        if report.type_export == 'pdf':
            ...
            data['form'] = self.read(cr, uid, ids, ['account_report_id', 'date_from_cmp',  'date_to_cmp',  'fiscalyear_id_cmp', 'journal_ids', 'period_from_cmp', 'period_to_cmp',  'filter_cmp',  'chart_account_id', 'target_move', 'level_report'], context=context)[0]
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        data['form'].update(self.read(cr, uid, ids, ['date_from_cmp',  'debit_credit', 'date_to_cmp',  'fiscalyear_id_cmp', 'period_from_cmp', 'period_to_cmp',  'filter_cmp', 'account_report_id', 'enable_filter', 'label_filter','target_move', 'level_report'], context=context)[0])
        ...
        for report in self.pool.get('account.financial.report').browse(cr, uid, ids2, context):
            if report.type == 'account_report':
                ...
                if data['form']['enable_filter']:
                    vals['balance_cmp'] = self.pool.get('account.financial.report').browse(cr, uid, report.id,
                                                                                           context=self.get_comparison_context(data['form']['comparison_context'])).balance * report.sign or 0.0
                lines.append(vals)
            account_ids = []
    ...
    def action_excel(self, cr, uid, ids, context=None):
        ...
        # ws.write(xi, 1, ' ', style_header)
        # ws.write(xi, 2, ' ', style_header)
        # if data['form']['debit_credit']:
        #     ws.write(xi, 3, ' ', style_header)
        #     ws.write(xi, 4, ' ', style_header)
        # ws.write(xi, 5, ' ', style_header)
        ...
        rf = rr = ri = 0
        for linea in lines:
            ws.write(xi, 0, linea.get('code', ''), linea_izq if linea.get('account_type') != 'view' else view_style)
            if linea:
                ws.write(xi, 1, linea.get('name', ''), linea_izq if linea.get('account_type') != 'view' and linea.get(
                        'type') == 'account' else view_style)
                # if len(linea.get('code', '')) == 2:
                #     ws.write(xi, 2, linea.get('name', ''), linea_izq if linea.get('account_type') != 'view' and linea.get(
                #         'type') == 'account' else view_style)
                # if len(linea.get('code', '')) == 4:
                #     ws.write(xi, 3, linea.get('name', ''), linea_izq if linea.get('account_type') != 'view' and linea.get(
                #         'type') == 'account' else view_style)
                # if len(linea.get('code', '')) == 6:
                #     ws.write(xi, 4, linea.get('name', ''), linea_izq if linea.get('account_type') != 'view' and linea.get(
                #         'type') == 'account' else view_style)
                # if len(linea.get('code', '')) == 8:
                #     ws.write(xi, 5, linea.get('name', ''), linea_izq if linea.get('account_type') != 'view' and linea.get(
                #         'type') == 'account' else view_style)

            # if data['form']['debit_credit']:
            #     ws.write(xi, 3, linea.get('debit', ''), linea_der)
            #     ws.write(xi, 4, linea.get('credit', ''), linea_der)
            if len(linea.get('code', '')) == 1:
                ws.write(xi, 6, linea.get('balance', ''), linea_der)
            if len(linea.get('code', '')) < 4 and len(linea.get('code', '')) > 2:
                ws.write(xi, 5, linea.get('balance', ''), linea_der)
            if len(linea.get('code', '')) < 6 and len(linea.get('code', '')) > 4:
                ws.write(xi, 4, linea.get('balance', ''), linea_der)
            if len(linea.get('code', '')) < 8 and len(linea.get('code', '')) > 6:
                ws.write(xi, 3, linea.get('balance', ''), linea_der)
            if len(linea.get('code', '')) < 10 and len(linea.get('code', '')) > 8:
                ws.write(xi, 2, linea.get('balance', ''), linea_der)
            if data['form']['enable_filter']:
                ws.write(xi, 6, linea.get('balance_cmp', ''), linea_der)
            col = 8
            xi += 1
...
class account_report_general_ledger2(osv.osv_memory):
    ...
    def action_excel(self, cr, uid, ids, chart_accountc_id, context=None):
        ...
        wb = xlsxwriter.Workbook('/opt/temp/Libro_Mayor.xlsx')
        ...
        style_header = wb.add_format({
            ...
            'align': 'center',
            'valign': 'vcenter'})
        style_header.set_font_name('Arial')
        style_header.set_font_size(10)

        linea_der= wb.add_format({
            'align': 'rigth',
            'valign': 'rigth'})
        linea_der.set_font_name('Arial')
        linea_der.set_font_size(7.5)

        linea_izquierda = wb.add_format({
            'align': 'left',
            'valign': 'rigth'})
        linea_izquierda.set_font_name('Arial')
        linea_izquierda.set_font_size(7.5)

        linea_center = wb.add_format({
            'align': 'center',
            'valign': 'rigth'})
        linea_center.set_font_name('Arial')
        linea_center.set_font_size(7.5)

        line_saldo = wb.add_format({
            'bold': 1,
            'align': 'rigth',
            'valign': 'rigth'})
        line_saldo.set_font_name('Arial')
        line_saldo.set_font_size(7.5)
        line_saldo_izq = wb.add_format({
            'bold': 1,
            'align': 'left',
            'valign': 'rigth'})
        line_saldo_izq.set_font_name('Arial')
        line_saldo_izq.set_font_size(7.5)

        style_cabecera = wb.add_format({
                        'bold': 1,
                        'align': 'center',
                        'valign': 'vcenter'})
        style_cabecera.set_font_name('Arial')
        style_cabecera.set_font_size(10)
        ...
        ws.merge_range(1, 1, 1, 12, compania.name, style_cabecera)
        ws.merge_range(2, 1, 2, 12, 'Direccion: ' + compania.partner_id.street, style_cabecera)
        ws.merge_range(3, 1, 3, 12, 'Ruc: ' + compania.partner_id.part_number, style_cabecera)
        ws.merge_range(5, 1, 5, 12, datetime.today().strftime('%Y-%m-%d'), style_cabecera)
        ...
        ws.write(xi, 3, 'No. Ch/Trans',style_header)
        ws.write(xi, 4, 'Empresa', style_header)
        ws.write(xi, 5, 'Ref', style_header)
        ws.write(xi, 6, 'Movimiento', style_header)
        ws.write(xi, 7, 'Etiqueta', style_header)
        ws.write(xi, 8, 'Contrapartida', style_header)
        ws.write(xi, 9, 'C.Costo', style_header)
        ws.write(xi, 10, 'Debito', style_header)
        ws.write(xi, 11, 'Credito', style_header)
        ws.write(xi, 12, 'Balance', style_header)
        ws.write(xi, 13, 'Moneda', style_header)
        ...
        for children in children_accounts:
            ...
            ws.write(xi, 1, children.code, line_saldo_izq)
            ws.write(xi, 2, children.name, line_saldo_izq)
            ws.write(xi, 10, self._sum_debit_account(cr, uid, report, query, children), line_saldo)
            ws.write(xi, 11, self._sum_credit_account(cr, uid, report, query, children), line_saldo)
            ws.write(xi, 12, self._sum_balance_account(cr, uid, report, query, children), line_saldo)
            # col = 8
            xi += 1
            for line in self.lines(cr, uid, report, query, children):
                ws.write(xi, 1, line.get('ldate', ''), linea_izquierda)
                ws.write(xi, 2, line.get('lcode', ''), linea_izquierda)
                ws.write(xi, 3, line.get('lno_cheque', ''), linea_izquierda)
                ws.write(xi, 4, line.get('lbenef') or line.get('lpartner'), linea_izquierda)
                ws.write(xi, 5, line.get('lref', ''), linea_center)
                ws.write(xi, 6, line.get('move', ''), linea_center)
                ws.write(xi, 7, line.get('lname', ''), linea_center)
                ws.write(xi, 8, line.get('line_corresp', ''), linea_center)
                ws.write(xi, 9, line.get('accon_name'), linea_der)
                ws.write(xi, 10, line.get('debit', ''), linea_der)
                ws.write(xi, 11, line.get('credit', ''), linea_der)
                ws.write(xi, 12, line.get('progress', ''), linea_der)
                xi += 1


        ws.set_column(0,0,6)  # 2000
        ws.set_column(1,1,17)  # 5800
        ws.set_column(2,2,29)  # 9900
        ws.set_column(3,3,45)  # 15000
        ws.set_column(4,4,17)  # 15000
        ws.set_column(5,5,19)  # 6500
        ws.set_column(6,7,45)  # 15000
        ws.set_column(8,9,16)  # 5500
        ws.set_column(10,30,18)  # 6500

        ws.set_row(8,40)

        try:
            wb.close()
            var = open('/opt/temp/Libro_Mayor.xlsx', 'r')
            buf = cStringIO.StringIO(var.read())
            out = base64.encodestring(buf.getvalue())
            data_fname = "Libro_Mayor.xlsx"  # "Libro_Mayor%s.xlsx"
            archivo = '/opt/temp/' + data_fname
    ...
    def lines(self, cr, uid, report, query, account):
        ...
        sql = """
            SELECT l.id AS lid, l.no_cheque AS lno_cheque, p.name AS lpartner, l.benef AS lbenef, l.date AS ldate, j.code AS lcode, l.currency_id,l.amount_currency,l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, l.period_id AS lperiod_id, l.partner_id AS lpartner_id,
