...
def crearXML(doc='Unknow', url='/opt/data/'):    # Me crea el fichero con la fecha del período '/data/110G/odooec/temp/'
        ...
        for h in brw_codes.get('compras'):
            ...
            for t in k_list:
                ...
                for tag in h.get(t).keys():
                    ...
                    elif tag == 'estabModificado':
                        esta_mod = str(h.get(t).get(tag))
                    ...
                    elif tag == 'isReimbursement':
                        is_reimb = str(h.get(t).get(tag))
                ...
                if (tip_pro == '03' and t_comprobante != '15'):
                    tipo_prov = doc.createElement("tipoProv")
                    detalle_compras_form.appendChild(tipo_prov)
                    p_tipo_prov = doc.createTextNode('01')
                    tipo_prov.appendChild(p_tipo_prov)

                    den_pro = doc.createElement("denoProv")
                    detalle_compras_form.appendChild(den_pro)
                    pdeno_prov = doc.createTextNode(str(deno_prov.replace('.',' ')))
                    den_pro.appendChild(pdeno_prov)
                ...
                base_no_gravada = Decimal(bas_imp_gra).quantize(Decimal('0.00'))
                ...
                f_base_no_gravada = Decimal(bas_ngi).quantize(Decimal('0.00'))
                ...
                tot_bas_reem = 0.00
                if is_reimb:
                    invdet_idr = self.pool.get('reimbursement.account').search(cr, uid, [('account_reimbursement_id', '=', idfac)])
                    invdet_detr = self.pool.get('reimbursement.account').browse(cr, uid, invdet_idr)
                ...
                if ((f_base_no_gravada >= 1000.00) or (f_base_imponible_grava >= 1000.00) or (base_no_gravada>= 1000.00) or (f_base_no_gravada + f_monto_iva >= 1000.00) or (base_no_gravada + f_monto_iva >= 1000.00) or (f_base_imponible_grava + f_monto_iva >= 1000.00) or (f_base_imponible_grava + f_base_no_gravada >= 1000.00) or (f_base_imponible_grava + f_base_no_gravada + f_monto_iva >= 1000.00) or (base_no_gravada + f_base_imponible_grava + f_base_no_gravada + f_monto_iva >= 1000.00)) :
                    ...
                    pf_fpago = doc.createTextNode("20")
                ...
                if (t_comprobante == '04'):
                    ...
                    pdocModificado = doc.createTextNode(str(doc_mod))
                    ...
                    pestabModificado = doc.createTextNode(str(esta_mod))
                ...
                if ((t_comprobante != '04') and (t_comprobante != '15')) :
                    ...
                    retenciones_332 = self.pool.get('account.invoice.tax').search(cr, uid, [('invoice_id', '=', idfac), ('deduction_id', '=', None), ('base_code_id.code', 'in', ['332','332I','332B','332G' ])])
                    retenciones_idr.extend([element for element in retenciones_332 if element not in retenciones_idr])
                    ...
                    for detalle_retencion in detalle_retenciones:
                        ...
                        elif codigo_ret == '332I':
                            val_por = 0.0
                            val_base = abs(detalle_retencion.base_amount) or 0.00
                            val_reten = abs(detalle_retencion.tax_amount) or 0.00
                        elif codigo_ret == '332B':
                            val_por = 0.0
                            val_base = abs(detalle_retencion.base_amount) or 0.00
                            val_reten = abs(detalle_retencion.tax_amount) or 0.00
                        elif codigo_ret == '332G':
                            val_por = 0.0
                            val_base = abs(detalle_retencion.base_amount) or 0.00
                            val_reten = abs(detalle_retencion.tax_amount) or 0.00
                ...
                if is_reimb:
                    invdet_id = self.pool.get('reimbursement.account').search(cr, uid, [('account_reimbursement_id', '=', idfac)])
                    invdet_det = self.pool.get('reimbursement.account').browse(cr, uid, invdet_id)
                    if invdet_det:
                        detalle_treemb_form = doc.createElement("reembolsos")
                        detalle_compras_form.appendChild(detalle_treemb_form)

                        for det_impf in invdet_det:
                            id_line=det_impf.id
                            print "ID LINE", id_line
            #                print "DESCRIPTION", str(det_impf.description.encode('UTF-8')).strip()
                            detalle_treemb = doc.createElement("reembolso")
                            detalle_treemb_form.appendChild(detalle_treemb)

                            tipcomreemb = doc.createElement("tipoComprobanteReemb")
                            detalle_treemb.appendChild(tipcomreemb)
                            ntipcomreemb = doc.createTextNode(str('01'))
                            tipcomreemb.appendChild(ntipcomreemb)

                            tipidprovreemb = doc.createElement("tpIdProvReemb")
                            detalle_treemb.appendChild(tipidprovreemb)
                            ntipidprovreemb = doc.createTextNode(str('01'))
                            tipidprovreemb.appendChild(ntipidprovreemb)

                            idprovreemb = doc.createElement("idProvReemb")
                            detalle_treemb.appendChild(idprovreemb)
                            nidprovreemb = doc.createTextNode(str(det_impf.ruc_ident_refund).strip())
                            idprovreemb.appendChild(nidprovreemb)

                            estareemb = doc.createElement("establecimientoReemb")
                            detalle_treemb.appendChild(estareemb)
                            nestareemb = doc.createTextNode(str(det_impf.est_refund).strip())
                            estareemb.appendChild(nestareemb)

                            ptoemireemb = doc.createElement("puntoEmisionReemb")
                            detalle_treemb.appendChild(ptoemireemb)
                            nptoemireemb = doc.createTextNode(str(det_impf.refund_series).strip())
                            ptoemireemb.appendChild(nptoemireemb)

                            secreemb = doc.createElement("secuencialReemb")
                            detalle_treemb.appendChild(secreemb)
                            nsecreemb = doc.createTextNode(str(det_impf.refund_number[8:]).strip())
                            secreemb.appendChild(nsecreemb)

                            fechemireemb = doc.createElement("fechaEmisionReemb")
                            detalle_treemb.appendChild(fechemireemb)
                            nfechemireemb = doc.createTextNode(time.strftime('%d/%m/%Y', time.strptime(det_impf.date_refund, '%Y-%m-%d')))
                            fechemireemb.appendChild(nfechemireemb)

                            autreemb = doc.createElement("autorizacionReemb")
                            detalle_treemb.appendChild(autreemb)
                            nautreemb = doc.createTextNode(str(det_impf.refund_authorization).strip())
                            autreemb.appendChild(nautreemb)

                            basimpceroreemb = doc.createElement("baseImponibleReemb")
                            detalle_treemb.appendChild(basimpceroreemb)
                            v_nbasimpceroreemb = Decimal(det_impf.refund_base_iva_cero).quantize(Decimal('0.00'))
                            nbasimpceroreemb = doc.createTextNode(str(v_nbasimpceroreemb).strip())
                            basimpceroreemb.appendChild(nbasimpceroreemb)

                            basimpgravreemb = doc.createElement("baseImpGravReemb")
                            detalle_treemb.appendChild(basimpgravreemb)
                            f_valor_basimpgravreemb = Decimal(det_impf.refund_base_iva).quantize(Decimal('0.00'))
                            nbasimpgravreemb = doc.createTextNode(str(f_valor_basimpgravreemb).strip())
                            basimpgravreemb.appendChild(nbasimpgravreemb)

                            basimpnoreemb = doc.createElement("baseNoGraIvaReemb")
                            detalle_treemb.appendChild(basimpnoreemb)
                            valor_basimpnogravreemb = Decimal(det_impf.refund_iva).quantize(Decimal('0.00'))
                            nbasimpnoreemb = doc.createTextNode(str(valor_basimpnogravreemb).strip())
                            basimpnoreemb.appendChild(nbasimpnoreemb)

                            basimpexereemb = doc.createElement("baseImpExeReemb")
                            detalle_treemb.appendChild(basimpexereemb)
                            nbasimpexereemb = doc.createTextNode(str('0.00'))
                            basimpexereemb.appendChild(nbasimpexereemb)

                            monticereeb = doc.createElement("montoIceRemb")
                            detalle_treemb.appendChild(monticereeb)
                            f_valor_monticereeb = Decimal(det_impf.ice_amount).quantize(Decimal('0.00'))
                            nmonticereeb = doc.createTextNode(str(f_valor_monticereeb).strip())
                            monticereeb.appendChild(nmonticereeb)

                            montivareeb = doc.createElement("montoIvaRemb")
                            detalle_treemb.appendChild(montivareeb)
                            f_valor_montivareeb = Decimal(det_impf.active_refund_iva).quantize(Decimal('0.00'))
                            nmontivareeb = doc.createTextNode(str(f_valor_montivareeb).strip())
                            montivareeb.appendChild(nmontivareeb)
...
def grabarXML(brw_codes, header, head_static, is_ats, doc='Unknow', url='/opt/data/'): # Guardar la información en el fichero
    archi=open(str(url) + str(doc) + '.xml','w')
    # Escribo en el XML
...
def grabarXMLATS(self, cr, uid, brw_codes, header, head_static, is_ats, doc='Unknow', url='/opt/data/'): # Guardar la información en el fichero
    archi=open(str(url) + str(doc) + '.xml','w')
    # Escribo en el XML
    #archi.write('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n')
