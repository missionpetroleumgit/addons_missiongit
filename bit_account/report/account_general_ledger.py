...
class general_ledger_ndf(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        used_ctx = data['form'].get('used_context',{})
        if data['form']['specific_account_id'] and data['form']['specific_account_id'] != data['form']['chart_account_id']:
            used_ctx['chart_account_id'] = data['form']['specific_account_id'][0]
        self.query = obj_move._query_get(self.cr, self.uid, obj='l', context=used_ctx)
        ...
        return super(general_ledger_ndf, self).set_context(objects, data, new_ids, report_type=report_type)

    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(general_ledger_ndf, self).__init__(cr, uid, name, context=context)
        self.query = ""
    ...
    def get_address(self):
           ...
           return user.company_id.street + ', ' + user.company_id.street2 + ', ' + user.company_id.city + ', ' + user.company_id.country_id.name

class report_generalledger(osv.AbstractModel):
    _name = 'report.bit_account.report_generalledger'
    _inherit = 'report.abstract_report'
    _template = 'bit_account.report_generalledger'
    _wrapped_report_class = general_ledger_ndf
