...
class account_invoice_indfe(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_discl':self.get_discl,
            'get_desct':self.get_desct,
    ...
    def get_discl(self, discount, price_unit, quantity):
        valor = 0.00
        if discount > 0:
            valor = round(((price_unit*quantity)*discount)/100, 2)
        else:
            valor = 0.00
        print "VALOR**************************************", valor
        return valor

    def get_desct(self, id):
        valor = 0.00
        id_acse = self.pool.get('account.invoice.line').search(self.cr, self.uid, [('invoice_id','=',id)])
        obj_acse = self.pool.get('account.invoice.line').browse(self.cr, self.uid, id_acse)
        for det_fact in obj_acse:
            if det_fact.discount > 0:
                valor += round(((det_fact.price_unit*det_fact.quantity)*det_fact.discount)/100, 2)
            else:
                valor = det_fact.discount
        print "VALOR TO DES", valor
        return valor
    ...
    def get_comprobante(self, nombre, entidad, emision, origen, fecha_inv):
        ...
        if nombre == 'out_invoice':
            ...
            id_invo = self.pool.get('account.invoice').search(self.cr, self.uid, [('number', '=', origen),('type', '=', 'out_invoice')])
            print "OJOO***", id_invo
            if len(id_invo) > 0:
                obj_id_invo = self.pool.get('account.invoice').browse(self.cr, self.uid, id_invo)[0]
                fecha_fact = obj_id_invo.date_invoice
                ent_dm =  str(obj_id_invo.authorization_id.serie_entity).strip()
                emi_dm =  str(obj_id_invo.authorization_id.serie_emission).strip()
                factura = str(obj_id_invo.number_seq).strip()
            ...
            fecha = 'FECHA EMISION(comprobante a modificar):'
            fecha1 = fecha_fact
            comprobante = str(ent_dm) +" - "+ str(emi_dm) +" - "+ str(factura).zfill(9)
            res = [title, fecha, fecha1, comprobante]
            print res
            return res
        else:
    ...
    def get_company(self):
        ...
        nombre = str(user.company_id.partner_id.name)
        print "nombre", nombre
