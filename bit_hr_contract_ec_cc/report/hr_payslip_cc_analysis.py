...
class hr_payslip_cc_analysis(osv.Model):
    _name = "hr.payslip.cc.analysis"
    _description = "Nomina por Centro de Costos"
    _auto = False
    _rec_name = 'number'
    _order = 'number'
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'hr_payslip_cc_analysis')
        cr.execute("""
            create or replace view hr_payslip_cc_analysis as (
                select 
                    min(pl.id) as id, 
                ...
                group by  r.order, paylist_name, account_analytic_id, number, pl.employee_id,empl.identification_id, empl.bank_account_id, employee_name, l.amount, salary_rule, pl.date_from, pl.date_to
                order by r.order
            )
        """)
