...
class purchase_order_invoice_report(models.TransientModel):
    _name = 'purchase.order.invoice.report'
    ...
    @api.multi
    def set_body(self, invoices, purchase, account, refund, ws, xi, linea_der, linea_izq, seq, linea_izq_n, linea_izq_neg, view_style, linea_der_bold
                 ,view_style_out):
        ...
        for order in invoices:
            if not refund:
                ...
                if purchase:
                    ws.write(xi, 2, purchase.name, linea_izq)
                    ws.write(xi, 3, purchase.date_order, linea_izq)
                else:
                    ws.write(xi, 2, '-----', linea_izq)
                    ws.write(xi, 3, '-----', linea_izq)
                ws.write(xi, 4, 'DC', linea_izq)
    ...
    def get_invoices(self):
        ...
        if self.account_analytic_id:
            ...
            domain = [('invoice_id.date_invoice', '>=', date_from), ('invoice_id.date_invoice', '<=', date_to),
                      ('invoice_id.state', 'in', ('open', 'paid')), ('type', '=', 'in_invoice')]
            ...
            account_invoice_data = account_invoice_line.search(domain, order='number_reem')
    ...
    def excel_action(self):
        ...
        if not self.account_analytic_id:
            for invoice in orders:
                    purchase_id = purchase_order_obj.search([('name', '=', invoice.origin)])
                    if purchase_id:
                        self.set_body(invoice, purchase_id, order, refund, ws, xi, linea_der, linea_izq, seq, linea_izq_n, linea_izq_neg,
                                      view_style, linea_der_bold, view_style_out)
                        xi += 1
                    else:
                        self.set_body(invoice, purchase_id, order, refund, ws, xi, linea_der, linea_izq, seq, linea_izq_n,
                                      linea_izq_neg,
                                      view_style, linea_der_bold, view_style_out)
                        xi += 1
                    for refund_id in invoice_ids:
                        if refund_id.id not in data:
                            ...
                            self.set_body(invoice, purchase_id, order, refund_id, ws, xi, linea_der, linea_izq, seq,
        ...
        if self.account_analytic_id:
            invoice = False
            for lines in orders:
                purchase_id = purchase_order_obj.search([('name', '=', lines.invoice_id.origin)])
                if purchase_id and invoice != lines.invoice_id.id:
                    self.set_body(lines.invoice_id, purchase_id, lines.analytics_id[0].name, ws, xi, linea_der, linea_izq, seq, linea_izq_n, linea_izq_neg,
                                  view_style, linea_der_bold, view_style_out)
                    invoice = lines.invoice_id.id
                    xi += 1
                else:
                    self.set_body(invoice, order, order, ws, xi, linea_der, linea_izq, seq, linea_izq_n,
                                  linea_izq_neg,
                                  view_style, linea_der_bold, view_style_out)
                    xi += 1
        ...
        try:
            ...
            buf.close()
            res_model = 'purchase.order.invoice.report'
